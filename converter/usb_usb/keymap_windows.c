...
const uint8_t keymaps[][MATRIX_ROWS][MATRIX_COLS] PROGMEM = {
    ...
    FN13,     TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,          TRNS,TRNS,TRNS,    TRNS,TRNS,TRNS,TRNS,    TRNS,
    TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS, FN4,     TRNS,TRNS,TRNS,    TRNS,TRNS,TRNS,TRNS,    TRNS,TRNS,
    FN1, FN10,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,     TRNS,     TRNS,TRNS,TRNS,    TRNS,TRNS,TRNS,TRNS,    TRNS,TRNS,
...
const action_t fn_actions[] PROGMEM = {
    ...
    [1] = ACTION_FUNCTION(1), // Start alt-tab
    ...
    switch (id) {
        case 0: // cmd press
            ...
            if (cmdPressed) {
                ...
                layer_off(4);
                host_setAltTabbing(false);
                del_weak_mods(MOD_BIT(KC_LCTL));
                unregister_mods(MOD_BIT(KC_LCTL));
                layer_off(1);
            }
            break;
        case 1: // Start alt-tab
            // TODO - esc out of alt-tab then cmd+c, etc. without releasing cmd
            if (record->event.pressed) {
                if (cmdPressed) host_setAltTabbing(true);
        ...
        case 3: // ctrl-left
            if (record->event.pressed) {
                ...
                if (optionPressed) {
                    register_mods(MOD_BIT(KC_LALT));
                    // optionPressed = false; // is this necessary? the two below lack it and seem fine.
                }
            }
            break;
        case 4: // ctrl-right
...
const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt)
{
    switch (id) {
        // Alt-Tab and hold
        case 0: return record->event.pressed ? MACRO(D(LALT), D(TAB), END) : MACRO(U(TAB), END);
        ...
        // Alt-F4
        case 5: return record->event.pressed ? MACRO(U(LCTL), D(LALT), T(F4), U(LALT), D(LCTL), END) : MACRO_NONE;
    }
    return MACRO_NONE;
}
